import DateSearch from "../DateSearch";
import GuestSearch from "./GuestSearch";
import LocationSearch from "./LocationSearch";
import { useNavigate } from "react-router-dom";
import { useEffect, useState } from "react";
import TimeSearch from "../TimeSearch";

const MainFilterSearchBox = () => {
  const navigate = useNavigate();

  const [currentTab, setCurrentTab] = useState("OUTSTATION");
  const tabs = [
    { id: 1, name: "OUTSTATION" },
    { id: 2, name: "AIRPORT" },
  ];

  // const OUTSTATION_SUB_TYPES = [
  //   { id: 1, name: "OUTSTATION" },
  //   { id: 2, name: "AIRPORT" },
  // ];

  return (
    <>
      <div className="tabs__controls d-flex x-gap-30 y-gap-20 justify-center sm:justify-start js-tabs-controls">
        {tabs?.map((tab) => (
          <button
            key={tab?.id}
            className={`tabs__button text-15 fw-500 text-white pb-4 js-tabs-button ${
              tab?.name === currentTab ? "is-tab-el-active" : ""
            }`}
            onClick={() => setCurrentTab(tab?.name)}
          >
            {tab?.name}
          </button>
        ))}
      </div>

      <div className="position-relative mt-30 md:mt-20 js-tabs-content">
        <div className="mainSearch -w-900 bg-white px-10 py-10 lg:px-20 lg:pt-5 lg:pb-20 rounded-100">
          <div className="button-grid items-center">
            {/* --From here - */}
            {currentTab == "OUTSTATION" ? (
              <>
                <div className="d-flex x-gap-30 y-gap-20 justify-center sm:justify-start">
                  {["ONE WAY", "ROUND WAY"]?.map((tab, index) => (
                    <div className="button-item">
                      <button
                        key={index}
                        className="button -dark-1 h-50 px-35 col-12 rounded-100 bg-blue-1 text-white"
                        // onClick={() => navigate("/hotel-list-v1")}
                      >
                        {tab}
                      </button>
                    </div>
                  ))}
                </div>
                {/* FROM  */}
                <LocationSearch cName="FROM" placeholder="Select City" />
                {/* End Location */}

                {/* TO  */}
                <LocationSearch cName="TO" placeholder="Select City" />

                <div className="searchMenu-date px-30 lg:py-20 lg:px-0 js-form-dd js-calendar">
                  <div>
                    <h4 className="text-15 fw-500 ls-2 lh-16">Date</h4>
                    <DateSearch />
                  </div>
                </div>
                {/* End Date search */}

                <div className="searchMenu-date px-30 lg:py-20 lg:px-0 js-form-dd js-calendar">
                  <div>
                    <h4 className="text-15 fw-500 ls-2 lh-16">Pick Up Time </h4>
                    <TimeSearch />
                  </div>
                </div>
                {/* time search end  */}
                {/* <GuestSearch /> */}
                {/* End guest */}
              </>
            ) : (
              <>
                <div className="d-flex x-gap-30 y-gap-20 justify-center sm:justify-start">
                  {["PICKUP", "DROP"]?.map((tab, index) => (
                    <div className="button-item">
                      <button
                        key={index}
                        className="button -dark-1 h-50 px-35 col-12 rounded-100 bg-blue-1 text-white"
                        // onClick={() => navigate("/hotel-list-v1")}
                      >
                        {tab}
                      </button>
                    </div>
                  ))} 
                </div>
               
                {/* FROM  */}
                <LocationSearch
                  cName="AIRPORT"
                  placeholder="Select Airport or City"
                />
                {/* End Location */}

                {/* TO  */}
                <LocationSearch
                  cName="PICKUP ADDRESS"
                  placeholder="Select Your Location"
                />

                <div className="searchMenu-date px-30 lg:py-20 lg:px-0 js-form-dd js-calendar">
                  <div>
                    <h4 className="text-15 fw-500 ls-2 lh-16">Date</h4>
                    <DateSearch />
                  </div>
                </div>
                {/* End Date search */}

                <div className="searchMenu-date px-30 lg:py-20 lg:px-0 js-form-dd js-calendar">
                  <div>
                    <h4 className="text-15 fw-500 ls-2 lh-16"> Time </h4>
                    <TimeSearch />
                  </div>
                </div>
                {/* time search end  */}
                {/* <GuestSearch /> */}
                {/* End guest */}
              </>
            )}

            {/* --To here - */}

            <div className="button-item">
              <button
                className="mainSearch__submit button -dark-1 h-60 px-35 col-12 rounded-100 bg-blue-1 text-white"
                onClick={() => navigate("/hotel-list-v1")}
              >
                <i className="icon-search text-20 mr-10" />
                Search Cabs
              </button>
            </div>
            {/* End search button_item */}
          </div>
        </div>
        {/* End .mainSearch */}
      </div>
      {/* End serarchbox tab-content */}
    </>
  );
};

export default MainFilterSearchBox;
